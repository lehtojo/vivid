...
public class MemberVariablePattern extends Pattern {
    ...
    public MemberVariablePattern() {
        // [private / protected / public] (id / id.id.id...id) (name)
    ...
    private TypeNode getTypeByIdentifier(Token token) {
        return Parser.getRoot().getType(((IdentifierToken)token).getIdentifier());
    }
    ...
    public Node build(Node parent, List<Token> tokens) throws Exception {
        ...
        if (token.getType() == TokenType.PROCESSED) {
            ...
            type = getTypeByIdentifier(token);
        }
