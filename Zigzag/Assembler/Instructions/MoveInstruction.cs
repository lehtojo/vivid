...
{
   public const string LOAD_ADDRESS = "lea";
   ...
   public override void OnBuild()
   {
      ...
      else if (Assembler.IsTargetX64 && Second.Value is DataSectionHandle handle && handle.Address)
      {
         ...
         var clone = handle.Finalize().To<DataSectionHandle>();
         clone.Address = false;
         ...
         Build(
            ...
            new InstructionParameter(
               new Result(clone, Second.Format),
               flags_second,
               HandleType.MEMORY
            )
         );
   ...
   public override void OnPostBuild()
   {
      ...
      {
         Operation = Source.Value.IsUnsigned ? UNSIGNED_CONVERSION : SIGNED_CONVERSION;
         return;
      }
